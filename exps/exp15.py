...
poisson_loss = torch.nn.PoissonNLLLoss(log_input=False, full=False, 
                                        size_average=None, eps=1e-08, 
                                        reduce=None, reduction='mean')
poisson_loss2 = torch.nn.PoissonNLLLoss(log_input=True, full=False, 
                                        size_average=None, eps=1e-08, 
                                        reduce=None, reduction='mean')

def criteria(output, target):
    aaa = poisson_loss2(output.flatten(), target)
    return aaa
# In[24]:


def petfinder_rmse(input,target):
    return F.mse_loss(torch.clip(torch.exp(input.flatten()), 1, 100), target)

def petfinder_rmse_prob(input,target):
    return petfinder_rmse(input,target)#100*torch.sqrt(F.mse_loss(input.flatten(), target))
...
for i in range(N_FOLDS):
    ...
    loss = petfinder_rmse_prob(val_preds, val_targets)
    losses.append(loss)
