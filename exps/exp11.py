...
train_df['norm_score'] = (train_df['norm_score'] + train_df['Pawpularity'] / 100) / 2
...
def get_learner(fold_num, model=None):
...
for i in range(N_FOLDS):
    print(f'= {i} results')
    model = create_model(model_path, pretrained=True, num_classes=1)

    if os.path.exists(f'{model_dir}/{model_path}_{i+1}.pth'):
        model.load_state_dict(torch.load(f'{model_dir}/{model_path}_{i}.pth'))
