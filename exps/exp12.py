...
def petfinder_rmse(input,target):
    pred = (torch.softmax(input, axis=1) * torch.arange(100).to(input.device)).sum(axis=1)
    loss = ((pred.cpu().numpy() - target.cpu().numpy()) ** 2).mean()
    return loss
...
def get_learner(fold_num):
    ...
    print('num_class', data.c)
    model = create_model(model_path, pretrained=True, num_classes=100)
    #model.load_state_dict(torch.load(f'{model_dir}/{model_path}_{fold_num}.pth'))
...
train_df['norm_score'] = train_df['path'].map(pd.read_csv('ens_exp4.csv', index_col='path')['pred']) 
train_df['norm_score'] = np.round(train_df['norm_score'] * 100).astype(int) - 1
# In[31]:
...
for i in range(N_FOLDS):
    ...
    pred = (torch.softmax(val_preds, axis=1) * torch.arange(100).to(val_preds.device)).sum(axis=1)
    train_df.loc[train_df['fold'] == i, 'pred'] = pred.cpu().numpy() + 1
