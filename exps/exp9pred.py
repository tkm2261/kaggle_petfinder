...
#if os.path.exists(model_dir + '/all_preds.npy'):
#    exit()
...
train_df['pred'] = 0
all_preds = np.zeros((train_df.shape[0], 100))
for i in range(N_FOLDS):
    ...
    all_preds[train_df['fold'] == i] = val_preds.cpu().numpy()


    pred = (val_preds * torch.arange(100).to(val_preds.device)).sum(axis=1)
    train_df.loc[train_df['fold'] == i, 'pred'] = (pred.cpu().numpy() + 1) / 100
...
# In[ ]:
np.save(model_dir + '/all_preds', all_preds)
